...
//  if (!empty($breadcrumb)) {
	...
	if (arg(1) !== NULL) {
	  if (is_numeric(arg(1))) {
	    ...
	    if (($node->type === 'patient') || ($node->type === 'media_gallery') || ($node->type === 'genetic_report') || ($node->type === 'gene_mutation')) {
	      $original = false;
	      $breadcrumb[0] = '<a href="' . $base_path . '">Home</a>';

	      if (is_current_user_owner($node)) {
		$breadcrumb[1] = '<a href="' . $base_path . 'mypatients">My patients</a>';
	      } else {
		$breadcrumb[1] = '<a href="' . $base_path . 'patients">Patients</a>';
	      }
	    }

	    if ($node->type === 'patient') {
	      $breadcrumb[2] = truncate_utf8(drupal_get_title(), theme_get_setting('breadcrumb_title_length'), $wordsafe = TRUE, $dots = TRUE);
	    }

	    if ($node->type === 'media_gallery') {
	      $patient_node = node_load($node->media_gallery_patient['und'][0]['nid']);
	      $breadcrumb[2] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '">' . $patient_node->title . '</a>';
	      $breadcrumb[3] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '/xrays">X-Rays</a>';
	      $breadcrumb[4] = truncate_utf8(drupal_get_title(), theme_get_setting('breadcrumb_title_length'), $wordsafe = TRUE, $dots = TRUE);
	    }
	    if ($node->type === 'genetic_report') {
	      $patient_node = node_load($node->field_genrep_patient['und'][0]['nid']);
	      $breadcrumb[2] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '">' . $patient_node->title . '</a>';
	      $breadcrumb[3] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '/genetic-reports">Genetic reports</a>';
	      $breadcrumb[4] = truncate_utf8(drupal_get_title(), theme_get_setting('breadcrumb_title_length'), $wordsafe = TRUE, $dots = TRUE);
	    }
	    if ($node->type === 'gene_mutation') {
	      ...
	      $breadcrumb[2] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '">' . $patient_node->title . '</a>';
	      $breadcrumb[3] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '/genetic-reports">Genetic reports</a>';
	      $breadcrumb[4] = '<a href="' . $base_path . 'node/' . $genrep_node->nid . '">' . $genrep_node->title . '</a>';
	      $breadcrumb[5] = truncate_utf8(drupal_get_title(), theme_get_setting('breadcrumb_title_length'), $wordsafe = TRUE, $dots = TRUE);
	    }
	  }
	}
      } else {
	if ((arg(0) === 'media-gallery') && (arg(1) === 'detail')) {
	  ...
	  // Weird bug: The link in the tag term overview page is wrong!!!
	  if ($node->type === 'x_ray') {
	    $xraycoll_node = node_load($node->field_xray_xraycollection['und'][0]['nid']);
	    $patient_node = node_load($xraycoll_node->media_gallery_patient['und'][0]['nid']);
	    $breadcrumb[2] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '">' . $patient_node->title . '</a>';
	    $breadcrumb[3] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '/xrays">X-Rays</a>';
	    $breadcrumb[4] = '<a href="' . $base_path . 'node/' . $xraycoll_node->nid . '">' . $xraycoll_node->title . '</a>';
	    $breadcrumb[5] = truncate_utf8(drupal_get_title(), theme_get_setting('breadcrumb_title_length'), $wordsafe = TRUE, $dots = TRUE);
	  } else {
	    if ($node->type === 'xray_description') {
	      $xray_node = node_load($node->field_xray_description_xray['und'][0]['nid']);
	      $xraycoll_node = node_load($xray_node->field_xray_xraycollection['und'][0]['nid']);
	      $patient_node = node_load($xraycoll_node->media_gallery_patient['und'][0]['nid']);
	      $breadcrumb[2] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '">' . $patient_node->title . '</a>';
	      $breadcrumb[3] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '/xrays">X-Rays</a>';
	      $breadcrumb[4] = '<a href="' . $base_path . 'node/' . $xraycoll_node->nid . '">' . $xraycoll_node->title . '</a>';
	      ...
	      $breadcrumb[2] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '">' . $patient_node->title . '</a>';
	      $breadcrumb[3] = '<a href="' . $base_path . 'node/' . $patient_node->nid . '/xrays">X-Rays</a>';
	      $breadcrumb[4] = '<a href="' . $base_path . 'node/' . $node->nid . '">' . $node->title . '</a>';
	      $breadcrumb[5] = truncate_utf8(drupal_get_title(), theme_get_setting('breadcrumb_title_length'), $wordsafe = TRUE, $dots = TRUE);
	    }
	  }
	}
	...
	if (arg(1) !== NULL) {
	  if (arg(1) === 'add') {
	    ...
	    $breadcrumb[0] = $variables['breadcrumb'][0];
	    $breadcrumb[1] = truncate_utf8(drupal_get_title(), theme_get_setting('breadcrumb_title_length'), $wordsafe = TRUE, $dots = TRUE);
	  }
	}
      }
    }
